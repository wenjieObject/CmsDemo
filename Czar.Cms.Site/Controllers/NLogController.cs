...
namespace Czar.Cms.Controllers
{
    ...
    {
        private INLogRepository NLogRepository;
        ...
        public Task<IActionResult> GetEntities()
        {
            ...
            {
                    var rows = NLogRepository.Get().ToList();
        ...
        public Task<IActionResult> GetEntitiesByPaged(int pageSize, int pageIndex)
        {
            return Task.Factory.StartNew<IActionResult>(() =>
            {
                var total = NLogRepository.Count(m => true);
                var rows = NLogRepository.GetByPagination(m => true, pageSize, pageIndex, true,
                    m => m.Id).ToList();
        ...
        public Task<IActionResult> Add(NLog model)
        {
            return Task.Factory.StartNew<IActionResult>(() =>
            {
                if(!ModelState.IsValid)
                    return Json(ExcutedResult.FailedResult("数据验证失败"));
                NLogRepository.AddAsync(model, false);
        ...
        public Task<IActionResult> Delete(int id)
        {
            return Task.Factory.StartNew<IActionResult>(() =>
            {
                NLogRepository.Delete(id, false);
