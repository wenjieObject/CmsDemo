...
namespace Czar.Cms.Controllers
{
    ...
    {
        private IAspNetUserLoginsRepository AspNetUserLoginsRepository;
        
        public AspNetUserLoginsController(IAspNetUserLoginsRepository repository)
        {
            AspNetUserLoginsRepository = repository ?? throw new ArgumentNullException(nameof(repository));
        }
        ...
        public Task<IActionResult> GetEntities()
        {
            ...
            {
                    var rows = AspNetUserLoginsRepository.Get().ToList();
        ...
        public Task<IActionResult> GetEntitiesByPaged(int pageSize, int pageIndex)
        {
            return Task.Factory.StartNew<IActionResult>(() =>
            {
                var total = AspNetUserLoginsRepository.Count(m => true);
                var rows = AspNetUserLoginsRepository.GetByPagination(m => true, pageSize, pageIndex, true,
                    m => m.Id).ToList();
        ...
        public Task<IActionResult> Add(AspNetUserLogins model)
        {
            return Task.Factory.StartNew<IActionResult>(() =>
            {
                if(!ModelState.IsValid)
                    return Json(ExcutedResult.FailedResult("数据验证失败"));
                AspNetUserLoginsRepository.AddAsync(model, false);
        ...
        public Task<IActionResult> Delete(int id)
        {
            return Task.Factory.StartNew<IActionResult>(() =>
            {
                AspNetUserLoginsRepository.Delete(id, false);
