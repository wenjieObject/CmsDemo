...
namespace Czar.Cms.Controllers
{
    ...
    {
        private IAspNetUserClaimsRepository AspNetUserClaimsRepository;
        
        public AspNetUserClaimsController(IAspNetUserClaimsRepository repository)
        {
            AspNetUserClaimsRepository = repository ?? throw new ArgumentNullException(nameof(repository));
        }
        ...
        public Task<IActionResult> GetEntities()
        {
            ...
            {
                    var rows = AspNetUserClaimsRepository.Get().ToList();
        ...
        public Task<IActionResult> GetEntitiesByPaged(int pageSize, int pageIndex)
        {
            return Task.Factory.StartNew<IActionResult>(() =>
            {
                var total = AspNetUserClaimsRepository.Count(m => true);
                var rows = AspNetUserClaimsRepository.GetByPagination(m => true, pageSize, pageIndex, true,
                    m => m.Id).ToList();
        ...
        public Task<IActionResult> Add(AspNetUserClaims model)
        {
            return Task.Factory.StartNew<IActionResult>(() =>
            {
                if(!ModelState.IsValid)
                    return Json(ExcutedResult.FailedResult("数据验证失败"));
                AspNetUserClaimsRepository.AddAsync(model, false);
        ...
        public Task<IActionResult> Delete(int id)
        {
            return Task.Factory.StartNew<IActionResult>(() =>
            {
                AspNetUserClaimsRepository.Delete(id, false);
