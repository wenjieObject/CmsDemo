...
namespace Czar.Cms.Controllers
{
    public class ManagerController : Controller
    {
        ...
        {
            ManagerRepository = repository ?? throw new ArgumentNullException(nameof(repository));
        }
        ...
        public Task<IActionResult> GetEntities()
        {
            ...
            {
                    var rows = ManagerRepository.Get().ToList();
        ...
        public Task<IActionResult> GetEntitiesByPaged(int pageSize, int pageIndex)
        {
            return Task.Factory.StartNew<IActionResult>(() =>
            {
                var total = ManagerRepository.Count(m => true);
                var rows = ManagerRepository.GetByPagination(m => true, pageSize, pageIndex, true,
                    m => m.Id).ToList();
        ...
        public Task<IActionResult> Add(Manager model)
        {
            return Task.Factory.StartNew<IActionResult>(() =>
            {
                if(!ModelState.IsValid)
                    return Json(ExcutedResult.FailedResult("数据验证失败"));
                ManagerRepository.AddAsync(model, false);
        ...
        public Task<IActionResult> Edit(Manager model)
        {
            return Task.Factory.StartNew<IActionResult>(() =>
            {
                if (!ModelState.IsValid)
                    return Json(ExcutedResult.FailedResult("数据验证失败"));
                ManagerRepository.Edit(model, false);
                return Json(ExcutedResult.SuccessResult());
            });
        }
        /// <summary>
