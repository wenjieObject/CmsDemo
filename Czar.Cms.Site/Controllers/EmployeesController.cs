...
namespace Czar.Cms.Controllers
{
    public class EmployeesController : Controller
    {
        ...
        {
            EmployeesRepository = repository ?? throw new ArgumentNullException(nameof(repository));
        }
        ...
        public Task<IActionResult> GetEntities()
        {
            ...
            {
                    var rows = EmployeesRepository.Get().ToList();
        ...
        public Task<IActionResult> GetEntitiesByPaged(int pageSize, int pageIndex)
        {
            return Task.Factory.StartNew<IActionResult>(() =>
            {
                var total = EmployeesRepository.Count(m => true);
                var rows = EmployeesRepository.GetByPagination(m => true, pageSize, pageIndex, true,
                    m => m.Id).ToList();
        ...
        public Task<IActionResult> Edit(Employees model)
        {
            return Task.Factory.StartNew<IActionResult>(() =>
            {
                if (!ModelState.IsValid)
                    return Json(ExcutedResult.FailedResult("数据验证失败"));
                EmployeesRepository.Edit(model, false);
                return Json(ExcutedResult.SuccessResult());
            });
        }
        /// <summary>
        ...
        public Task<IActionResult> Delete(int id)
        {
            return Task.Factory.StartNew<IActionResult>(() =>
            {
                EmployeesRepository.Delete(id, false);
